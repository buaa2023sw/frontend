      ...
      <v-col cols="6">
        <v-container>
          <v-card class="fill-height">
            <v-card-title :style="getLinearGradient(user.topic)"><strong :style="'color : ' + getTopicColor(user.topic)">请输入需要生成测试数据的代码</strong></v-card-title>
            <v-divider></v-divider>
...
<!--              </v-textarea>-->
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn :color="getTopicColor(user.topic)" text :loading="resultBusy" :disabled="!valid()" @click="startTestData()">确定</v-btn>
            </v-card-actions>
          </v-card>
        </v-container>
      </v-col>
      <v-col cols="6">
        <v-container>
          <v-card class="fill-height">
            <v-card-title :style="getLinearGradient(user.topic)"><strong :style="'color : ' + getTopicColor(user.topic)">生成的测试数据</strong></v-card-title>
            <v-divider></v-divider>
            <v-card-text class="d-flex flex-column" style="width: 100%; height: 100%">
...
<!--              {{ result }}-->
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn :color="getTopicColor(user.topic)" text @click="download">下载测试回复</v-btn>
            </v-card-actions>
          </v-card>
        </v-container>
      </v-col>
  ...
  methods: {
    ...
    download() {
      this.downloadStrAsFile(
          ...
          `单元测试-${new Date().toLocaleString()}.md`)
